-- TODO write tests
-- TODO export the correct functions
...
-- TODO consider making a key data type
-- TODO maybe have the debug functions print to a specified file
-- TODO key data type
-- TODO key creation function
-- TODO move printbytes to the test file??
-- maybe use the doubleword as the key type??
-- TODO write comments ??
-- TODO there should be one cipher function for each diffrent keysize
-- additionally the keyExpansion functions should be private 
-- you should be able to toss in a key 
-- and get out a cipher that will encrypt and decrypt byte by byte
module AES 
(cipher, 
cipherDebug, 
keyExpansion, 
...
---------------------------------------------------------------------------------------------------
-- maybe change foldr
-- also maybe don't use !! operator
-- maybe use ffmult on all operations just for pretty code
...
mixColumn col =
  ...
  -- [ foldr ffAdd (xtime (col !! 0)) [(ffMultiply (col !! 1) 0x3), (col !! 2), (col !! 3)],
  --   foldr ffAdd (col !! 0) [(xtime (col !! 1)), (ffMultiply (col !! 2) 0x3), (col !! 3)],
  --   foldr ffAdd (col !! 0) [(col !! 1), (xtime (col !! 2)), (ffMultiply (col !! 3) 0x3)],
  --   foldr ffAdd (ffMultiply (col !! 0) 0x3) [(col !! 1), (col !! 2), (xtime (col !! 3))]
  -- ]
...
keyExpansionEIC :: [Word8] -> [[Word8]]
keyExpansionEIC keyBytes = front ++ map invMixColumns middle ++ rear
  where
    dw = keyExpansion keyBytes
    front = take 4 dw
...
cipher plain key = cipherRec (addRoundKey plain key 0) key 0
...
invCipher plain key = invCipherRec (addRoundKey plain key ((length key`div`4)-1)) key 0
...
eqInvCipher :: [Word8] -> [[Word8]] -> [Word8]
eqInvCipher plain key = eqInvCipherRec (addRoundKey plain key ((length key`div`4)-1)) key 0
...
main = do
  ...
  let expandedKey128 = keyExpansion key128
  let expandedKey192 = keyExpansion key192
  let expandedKey256 = keyExpansion key256
