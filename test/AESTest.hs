...
-- monads to bind maybe value
-- move consts to another file
...
makeBlockSpec = describe "makeBlock" $ do
  it "Returns Just AES.Block bit from a list of 16 Word8's" $
    showBlock (fromJust (makeBlock plainText)) `shouldBe` concatMap (printf "%02x") plainText
...
showBlockSpec = describe "showBlock" $ do
  it "Returns a String representation of a 128 bit AES.Block" $
    showBlock (fromJust (makeBlock plainText)) `shouldBe` "00112233445566778899aabbccddeeff"
